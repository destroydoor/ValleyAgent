...
> **TS 服务器**：`server\packages\stardew\bin\valley-ai-server.exe`（Bun 编译产物，源码在 `server\packages\stardew\src`）  
...
{
  "Provider": "MiniMax",
  "ServerExecutablePath": "server\\packages\\stardew\\bin\\valley-ai-server.exe",
  "ServerDirectory": "server\\packages\\stardew",
...
| 1.2 | 启动 Agent 服务器 | 主机端 TypeScript 服务器已启动并可连接，无 `connection refused`/`timeout`。服务器可执行文件位于 `server\packages\stardew\bin\valley-ai-server.exe`。 |
