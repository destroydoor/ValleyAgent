...
**探查 A：TS 侧"每玩家一个导演"真相**（源码在本仓 `server/`，主代码 `server/packages/stardew/src`）
...
**探查 B：C# 主机主线程阻塞/死锁审计**（`src/ValleyAgent` + `src/ValleyAgent.Abstractions`）
...
- TS 源码：本仓 `server/`（已并入本仓库，不再是兄弟目录）；C# 源码：本仓 `src/`。
...
- `dotnet test` 需 `DOTNET_ROOT="C:\Program Files\dotnet"`；TMP/TEMP 指向仓库根 `.tmp\`（禁落 C 盘）。
