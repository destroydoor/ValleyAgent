...
> ⚠️ **部分被推翻（2026-08-15）**：本文件 §2.1 的「**C# 是唯一的账本与状态持有者**」（钱包/背包在 C#）
> 及哲学第 6 条的账本归属，**已被 [`2026-08-15-ts-ledger-reflex-architecture.md`](2026-08-15-ts-ledger-reflex-architecture.md)
> 推翻**：经济/背包权威账本与情绪引擎已迁 TS，C# 只持执行镜像 + 物理校验 + adjust 原语执行（四步已全部落地）。
> 三层结构、状态光谱、记忆分层、Director 单向控制原则仍然有效。阅读本文件时请以修订文档为准。
>
> **Created:** 2026-08-05
