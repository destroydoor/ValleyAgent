...
// 若有人把静态段又加到尾部，此测试会失败。
// 2026-09-12：BEAT_SYSTEM_TEMPLATE（runBeat ReAct 接管）已随 runBeat 退役删除；
// L3 beat 改为对话模板内的条件段 {beat_section}（第三人称场景，无导演概念）。
// ---------------------------------------------------------------------------

test("DIALOGUE_SYSTEM_TEMPLATE segments ordered by volatility (static before dynamic)", () => {
  // 各段标识，按变动频率从低到高排列：
  // 静态 → 静态 → 静态 → 准静态 → 低频 → 低频(beat 窗口内稳定) → 中频(条件注入占位符)
  // → 高频 → 每轮必变 → 每轮必变
  const markers = [
    ...
    "{beat_section}",          // 5. L3 beat 场景（低频——beat 窗口内稳定，2026-09-12 新增）
    "{actual_state_section}",  // 6. 真实状态镜像（中频，条件注入占位符）
    "最近记忆",                // 7. 最近记忆（高频）
    "最近对话",                // 8. 最近对话（每轮必变）
    "当前场景",                // 9. 当前场景（每轮必变）
  ];
